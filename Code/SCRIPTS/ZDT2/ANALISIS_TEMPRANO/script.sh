...
echo "Para ejecutar este Script solamente es necesario modificar el archivo INPUT_DATA/zdt2.in"
...
n_gen=$(awk '{if(NR==2){print $0}}' INPUT_DATA/zdt2.in)
gen_grad=$(awk '{if(NR==5)final=5+$0+14; if(NR==final)print $0;}' INPUT_DATA/zdt2.in)
...
do
    ...
    awk -f AWK/zdt2_iter.awk INPUT_DATA/zdt2.in > INPUT_DATA/zdt2_iter.in
	...
	semilla=$(bc <<< 'scale=8; '$b' + (1-'$b')*('$i'-1)/'$n_eje'')
        ...
        ./nsga2r $semilla < INPUT_DATA/zdt2_iter.in > borrar.txt
	...
	awk -f AWK/archivo_nsga2.awk INPUT_DATA/zdt2.in > INPUT_DATA/zdt2_o.in
        ./nsga2r $semilla < INPUT_DATA/zdt2_o.in > borrar.txt
