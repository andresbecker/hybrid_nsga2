...
/*Variables globales para metodo del gradiente*/
int eval;
int eval_nsga2;
int eval_grad;
double epsilon;
...
{
    int i;
    /*Variable para contar numero de evaluaciones de la funcion objetivo*/
    eval=0;
    eval_nsga2=0;
    eval_grad=0;
    maxiter=50;
    ...
    /*DATOS DE ENTRADA PARA METODO DEL GRADIENTE*/
    if (choice==0)
    {
        scanf("%d", &i);
        scanf("%d", &i);
    }
    printf("\nINGRESE c1 PARA ARMIJO:\n");
    scanf("%lf", &c1);
    printf("\nINGRESE EPSILON CRITERIO DE PARADA:\n");
    scanf("%lf", &epsilon);
    printf("\nINGRESE 'h' PARA APROXIMAR GRADIENTE:\n");
    scanf("%lf", &h);
    printf("\nINGRESE 'alpha_maximo' PARA ARMIJO:\n");
    scanf("%lf", &alfa_max);
    printf( "Introduzca el maximo de iteraciones:\n" );
    scanf("%d", &maxiter);
    printf( "Introduzca la generacion donde se ejecutara el MDPM:\n" );
    scanf("%d", &gen_grad);
    ...
    for (i=2; i<=ngen; i++)
    {
        /*SELECCIONA PADRES Y LLEVA A CABO CRUZA PARA GENERAR POBLACION DE HIJOS*/
        selection (parent_pop, child_pop);
        mutation_pop (child_pop);
        decode_pop(child_pop);
        evaluate_pop(child_pop);
        /*MEZCLA POBLACIONES EN UNA PARA SELECCION*/
        merge (parent_pop, child_pop, mixed_pop);
        /*ESCOGE NUEVA POBLACION DE LA MEZCLADA*/
        fill_nondominated_sort (mixed_pop, parent_pop);
        /*LLEVA A CABO METODO ME MAXIMO DECSENSO*/
        if(gen_grad==i)
            metodo_grad(parent_pop);
    ...
    fprintf(fp_eval,"%d %d", eval_nsga2, eval_grad);
    fflush(stdout);
    fflush(fpt1);
    fflush(fpt2);
    fflush(fpt3);
    ...
    printf("\nNumero de Evaluaciones totales de la funcion objetivo=\n%d", eval_nsga2+eval_grad);
    printf("\nNumero de Evaluaciones de la funcion objetivo NSGA-II=\n%d", eval_nsga2);
    printf("\nNumero de Evaluaciones de la funcion objetivo MDPM=\n%d\n", eval_grad);
    return (0);
}
