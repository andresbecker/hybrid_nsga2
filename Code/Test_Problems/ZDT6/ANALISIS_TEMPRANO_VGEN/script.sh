...
echo "Para ejecutar este Script solamente es necesario modificar el archivo INPUT_DATA/zdt4.in"
...
#DETERMINAMOS A PARTIR DE QUE GENERACION EMPIEZA A ITERAR
gen_grad=$(awk '{if(NR==2) print $0;}' INPUT_DATA/zdt4.in)
...
do
    ...
    awk -f AWK/zdt4_iter.awk INPUT_DATA/zdt4.in > INPUT_DATA/zdt4_iter.in
	...
	semilla=$(bc <<< 'scale=8; '$b' + (1-'$b')*('$i'-1)/'$n_eje'')
        ...
        ./nsga2r $semilla < INPUT_DATA/zdt4_iter.in > borrar.txt
	...
	awk -f AWK/archivo_nsga2.awk INPUT_DATA/zdt4.in > INPUT_DATA/zdt4_o.in
        ./nsga2r $semilla < INPUT_DATA/zdt4_o.in > borrar.txt
